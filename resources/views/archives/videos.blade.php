            ...
            <div class="main-content-inner">
                                        ...
                                        <table class="table text-center">
                                            ...
                                            <tbody>
                                                @foreach ($Video as $Video)
                                                    <tr>
                                                        <td>{{ count(explode(',', $Video->selectfileVid)) }}</td>
                                                        <td>{{ $Video->nameVid }}</td>
                                                        <td>{{ $Video->textKey }}</td>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                <form action="{{ route('Videos.store') }}" method="POST" enctype="multipart/form-data" class="t-right">
                                    @csrf
                                    <div class="row">
                                        <div class="form-group col-lg-3 col-md-4 col-sm-12 pt-3 pb-3">
                                            <input class="form-control @error('nameVid') is-invalid @enderror" type="text" name="nameVid" placeholder="اسم الفيديو" value="{{ old('nameVid') }}">
                                            @error('nameVid')
                                                <p class="text-danger">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div class="form-group col-lg-3 col-md-4 col-sm-12 pt-3 pb-3">
                                            <select class="form-control @error('conditionVid') is-invalid @enderror" name="conditionVid">
                                                <option selected value="لم يتم التحديد"> .. يتبع الي </option>
                                                @foreach ($Sections as $Section)
                                                    <option value="{{ $Section->nameSection }}">
                                                        {{ $Section->nameSection }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group col-lg-3 col-md-4 col-sm-12 pt-3 pb-3">
                                            <select class="form-control @error('sideVid') is-invalid @enderror" name="sideVid">
                                        ...
                                        <div class="form-group col-lg-3 col-md-4 col-sm-12 pt-3 pb-3">
                                            <input class="form-control @error('selectfileVid') is-invalid @enderror" type="file" name="selectfileVid[]" multiple>
                                            @error('selectfileVid')
                                                <p class="text-danger">{{ $message }}</p>
                                            @enderror
                                        </div>
