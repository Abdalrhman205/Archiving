<div id="SearchFile">
    <div class="search-box pull-right">
        <div class="up-card t-right">
            <h3 class="c-grey pb-3">البحث عن الملفات</h3>
        </div>
        <form class="d-flex" role="search">
            <input wire:model.live="searchFile" class="form-control me-2" aria-label="Search" type="search" name="search" placeholder="Search..." required>
            <i class="ti-search"></i>
        </form>
                                            ...
                                            <tr onclick="tableFile({{ $file->id }})">
                                                <td>
                                                    @if ($file->selectfileFile)
                                                        @php
                                                            $fileExtension = pathinfo($file->selectfileFile, PATHINFO_EXTENSION);
                                                            $imageExtensions = ['jpg', 'jpeg', 'png', 'gif'];
                                                        @endphp

                                                        @if (in_array(strtolower($fileExtension), $imageExtensions))
                                                            <img src="{{ asset('images/' . $file->selectfileFile) }}" alt="Image" width="100" height="100">
                                                        @else
